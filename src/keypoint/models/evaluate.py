...
class LogImagePredictionPerNumEpochs(Callback):
    ...
    def _log_predictions(self, data_loader: Sequence, epoch):
        predictions = self.model.predict(data_loader)[:, :NUM_TARGETS]
        img_idx_per_batch = 3
        for batch_idx, data in enumerate(data_loader):
            ...
            scaled_prediction = np.array(
                rescale_kps_from_pct(_img_arr.shape, list(prediction))
            )
            scaled_ground_truth = np.array(
                rescale_kps_from_pct(_img_arr.shape, list(ground_truth))
            )
            # wandb specific code
            _original_img = Image.fromarray(_img_arr)
            _wandb_original_image = wandb.Image(_original_img.convert("RGB"))
            image_kwargs = {"radius": 1, "xy_offset": (3, 1), "font_size": 7}
            _wandb_ground_truth = wandb.Image(
                show_image_coords(
                    _img_arr, true_coords=scaled_ground_truth, **image_kwargs
                )
            )
            _wandb_predicted_image = wandb.Image(
                show_image_coords(
                    _img_arr, pred_coords=scaled_prediction, **image_kwargs
                )
            )
            _loss = np.linalg.norm(data[1][img_idx_per_batch] - prediction, axis=0)

            self.predictions_table.add_data(
                img_id,
                epoch,
                img_num,
                _wandb_original_image,
                _wandb_predicted_image,
                _wandb_ground_truth,
                _loss,
                wandb.run.id,
            )
...
def evaluate(model: Model, test_loader: KeyPointDataLoader):
    ...
    (
        ...
        predictions,
        hardest_k_names,
    ) = get_hardest_k_examples(model, test_loader)
...
if __name__ == "__main__":
    ...
    val_loader = KeyPointDataLoader(
        input_img_paths=imgs, target_paths=labels, batch_size=1, img_size=(128, 128)
    )
